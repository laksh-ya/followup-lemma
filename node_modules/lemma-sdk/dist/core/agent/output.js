...
function messageTextCandidates(message, options = {}) {
    ...
    const textParts = (message.parts || [])
        .filter((part) => ((part.type === "text" || (options.includeReasoning === true && part.type === "reasoning"))
...
function getMessageMetadata(message) {
    ...
    if (isRecord(message.message_metadata))
...
function finalOutputFromMetadata(messages) {
    for (const message of latestFirst(messages)) {
        ...
        if (metadata?.is_final_answer !== true && metadata?.isFinalAnswer !== true)
            continue;
...
function finalOutputFromToolInvocations(messages) {
    ...
    for (const invocation of [...toolInvocations].reverse()) {
        ...
        if (!isFinalResultToolName(toolName))
            continue;
 ...
 * the assistant text.
 */
export function extractAgentFinalOutput(messages, options = {}) {
